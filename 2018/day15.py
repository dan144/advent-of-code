...
def calculate_score(rounds, elves, goblins):
    print('Rounds:', rounds)
    hp_sum = 0
    if elves:
        print('Elves win')
...
while run:
    ...
    while r_inc:
                    ...
                    if target_locs and not dont:
                        ...
                        while points:
                            ...
                            for point in points:
                                ...
                                for xoff, yoff in {(-1, 0), (0, -1), (0, 1), (1, 0)}:
                                    if not dboard[py+yoff][px+xoff] and str(board[py+yoff][px+xoff]) == '.' and (px+xoff, py+yoff) not in npoints:
                        ...
                        if closest_locs:
                            ...
                            while points:
                                ...
                                for point in points:
                                    ...
                                    for xoff, yoff in {(-1, 0), (0, -1), (0, 1), (1, 0)}:
                                        if not dboard[py+yoff][px+xoff] and str(board[py+yoff][px+xoff]) in '.+' and (px+xoff, py+yoff) not in npoints:
                                            npoints.append((px+xoff, py+yoff))
                                points = npoints
                        ...
                        for e in in_range:
                            if min_e is None:
                                ...
                                if e.y < min_e.y:
                                    ...
                                    min_e = e
                                    min_hp = e.hp
                        min_e.hp -= GAP if type(me) == Goblin else EAP
                        if min_e.hp <= 0:
        ...
        #show_board()
        #input()
        if EAP > 3 and nelves != len(elves):
            print('Lost elves with elf power', EAP)
            EAP +=1
            break
    ...
    if EAP == 3:
        ans1 = calculate_score(rounds, elves, goblins)
        print(ans1)
        if testing:
            if part_one == ans1:
        ...
        print('PART TWO')
    if nelves == len(elves):
        print('Saved all them elves with elf power', EAP)
        ans2 = calculate_score(rounds, elves, goblins)
        run = False
